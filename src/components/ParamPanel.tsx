    ...
    <div className="flex flex-col gap-5">
      ...
      {isDmfox && !inputImages.length && (
        <section>
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 text-[11px] text-amber-700 leading-relaxed">
            ...
            添加参考图可使用 OpenAI 标准 <code className="bg-amber-100/50 px-1 rounded">/v1/images/edits</code> 端点。
          </div>
        </section>
      )}
      ...
      {isDmfox && inputImages.length > 0 && (
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-2 text-[10px] text-blue-600 leading-relaxed">
          参考图将通过标准 <code className="bg-blue-100/50 px-1 rounded">/v1/images/edits</code> 端点提交。
          {inputImages.length > 1 && (
            <span className="block mt-1 text-amber-600">多张参考图将自动合成为一张拼图后再提交。</span>
          )}
        </div>
      )}

      {/* 遮罩图（局部重绘） */}
      {(isApimart || isDmfox) && <section>
        <div className="flex items-center justify-between mb-2">
        ...
        {maskImage ? (
          ...
          <div className="flex gap-2">
            <div
              ...
              className="flex-1 border border-dashed border-gray-200 rounded-lg py-2.5 flex flex-col items-center gap-1 cursor-pointer hover:border-purple-300 hover:bg-purple-50/50 transition-colors"
            >
            ...
            {inputImages.length > 0 && (
              <button
                ...
                className="flex-1 border border-dashed border-purple-200 rounded-lg py-2.5 flex flex-col items-center gap-1 cursor-pointer hover:border-purple-300 hover:bg-purple-50/50 transition-colors"
              >
                <svg className="w-4 h-4 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                </svg>
                <span className="text-[10px] text-purple-400">绘制</span>
              </button>
            )}
          </div>
        )}
        ...
        {inputImages.length > 0 && maskImage && (
          <div className="mt-2 p-2 bg-purple-50/50 border border-purple-100 rounded-lg">
            <p className="text-[10px] text-purple-600 leading-relaxed">
              遮罩图尺寸需与首张参考图一致。透明区域将被重绘，不透明区域保留原图。
            </p>
          </div>
        )}
        {inputImages.length === 0 && maskImage && (
